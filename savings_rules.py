...
def get_simple_linear(
    initial_rrsp: float, final_rrsp: float, initial_year: int, career_length_yrs: int
):
    """
    ...
    """
    return get_simple_linear_func(
        lambda: initial_rrsp, lambda: final_rrsp, initial_year, career_length_yrs, None
    )


def get_simple_linear_func(
    initial_rrsp_func: Callable[[], float],
    final_rrsp_func: Callable[[], float],
    initial_year: int,
    career_length_yrs: int,
    fail_func: Callable[[], None],
):
    """
    ...
    def simple_linear(
        ...
        if not (0 <= initial_rrsp <= 1):
            if fail_func != None:
                fail_func(
                    "savings_rules.simple_linear: initial_rrsp must be between 0 and 1"
                )
            else:
        ...
        if not (0 <= final_rrsp <= 1):
            if fail_func != None:
                fail_func(
                    "savings_rules.simple_linear: final_rrsp must be between 0 and 1"
                )
            else:
        ...
        if not (0 <= years_elapsed <= career_length_yrs):
            raise ValueError(
                f"{deltas.year} lies outside the allowed range of years for the rule (initial year={initial_year}, career length={career_length_yrs})"
            )
        ...
        if fail_func != None and not is_in_bounds:
            fail_func(
                "savings_rules.simple_linear: interpolated RRSP must be between 0 and 1"
            )
        else:
            assert is_in_bounds
...
def get_simple_retirement_deduction(retirement_year: int, year_of_death: int):
    ...
    def simple_retirement_deduction(
        deltas: model.deltas_state,
        previous_funds: model.funds_state,
        previous_deltas: model.deltas_state,
    ):
        ...
        if years_elapsed < 0 or deltas.year > year_of_death:
            raise ValueError(
                f"{deltas.year} lies outside the allowed range of years for the rule (initial year={retirement_year}, final year={year_of_death})"
            )

        #
        spending = (
            -deltas.undifferentiated_savings
        )  # We expect undifferentiated_savings to be a negative value, with contributions from
        # spending (retirement income) + tax owed on last year's RRSP withdrawal
        remaining_rrsp = previous_funds.rrsp_savings
        rrsp_allotment = remaining_rrsp / (
            years_remaining + 1
        )  # Try to distribute RRSP withdrawals evenly to minimize marginal tax
        rrsp_withdrawal = max(
            min(spending, rrsp_allotment), 0
        )  # Don't let the RRSP go below 0. This is mainly to try to cut down on weird edge
...
def get__linear_retirement_deduction_func(
    initial_rrsp_func: Callable[[], float],
    final_rrsp_func: Callable[[], float],
    initial_year: int,
    retirement_length_yrs: int,
    fail_func: Callable[[], None],
):
    """
    ...
    """
    inner_rule = get_simple_linear_func(
        initial_rrsp_func,
        final_rrsp_func,
        initial_year,
    ...
    def checked_rule(
        ...
        if previous_funds.rrsp_savings + deltas.rrsp < 0:
            #
            fail_func(
                "savings_rules.linear_retirement_deduction: RRSP must not go below 0"
            )
        return output
...
def get_adjusted_heuristic_retirement_deduction(
    retirement_year: int, year_of_death: int, rrsp_adjustment_func: Callable[[], float]
):
    """
    Deduct from savings to cover retirement income. The split between RRSP and TFSA is made by a simple heuristic which tries to keep a
