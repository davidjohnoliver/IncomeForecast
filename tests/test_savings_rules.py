...
def test_simple_retirement_deduction_excess_tfsa():
    rule = savings_rules.get_simple_retirement_deduction(
        retirement_year=2040, year_of_death=2060
    )
    deltas_2041 = model.deltas_state.from_year(2041)
    funds_2040 = model.funds_state(320000, 550000, 2040, 0.0, 0.0, 0.0)
...
def test_simple_retirement_deduction_exact_savings():
    ...
    funds_2040 = model.funds_state(320000, 480000, 2040, 0.0, 0.0, 0.0)
    ...
    for y in range(retirement_year, year_of_death):
        year = y + 1
        deltas = model.deltas_state.from_year(year).update_spending(spending)
        deltas = rule(deltas, previous_funds, previous_deltas)
...
def test_linear_retirement_deduction_func():
    rule = savings_rules.get__linear_retirement_deduction_func(
        lambda: 0.2, lambda: 0.8, 2022, 10, None
    )

    SPENDING = 40000

    bottomless_funds = model.funds_state(
        rrsp_savings=1000000,
...
def test_adjusted_heuristic_retirement_deduction_excess_tfsa():
    rule = savings_rules.get_adjusted_heuristic_retirement_deduction(
        retirement_year=2040, year_of_death=2060, rrsp_adjustment_func=lambda: 0
    )
    deltas_2041 = model.deltas_state.from_year(2041)
    funds_2040 = model.funds_state(320000, 550000, 2040, 0.0, 0.0, 0.0)
...
def test_adjusted_heuristic_retirement_deduction_exact_savings():
    retirement_year = 2040
    year_of_death = 2060
    rule = savings_rules.get_adjusted_heuristic_retirement_deduction(
        retirement_year, year_of_death, rrsp_adjustment_func=lambda: 0
    )
    funds_2040 = model.funds_state(320000, 480000, 2040, 0.0, 0.0, 0.0)
