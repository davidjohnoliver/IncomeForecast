...
def test_get_luxury_over_basic():
    ...
    new_delta = model.get_updated_deltas_from_rules(
        standard_previous_funds(),
        standard_previous_deltas().update_spending(50000),
        [rule],
    )
...
def test_get_maxed_or_zeroed_out():
    ...
    assert math.isclose(
        c_m, spending_rules.get_maxed_or_zeroed_out(0.9, c_m, end_region)
    )
