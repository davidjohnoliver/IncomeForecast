...
def _update_higher_and_lower(deltas: model.couple_deltas_state, higher_delta: model.deltas_state, lower_delta: model.deltas_state):
    if deltas.partner2_deltas.gross_salary > deltas.partner1_deltas.gross_salary:
        return deltas.update_partner2_deltas(higher_delta).update_partner1_deltas(lower_delta)
    else:
        return deltas.update_partner1_deltas(higher_delta).update_partner2_deltas(lower_delta)
...
def get_equalizing_rrsp_only_split():
    ...
    def equalizing_rrsp_only_split(deltas: model.couple_deltas_state, previous_funds: model.couple_funds_state, previous_deltas: model.couple_deltas_state):
        raw_savings = deltas.household_undifferentiated_savings
        if raw_savings >= 0: # The couple is 'functionally working', ie their salaried net income exceeds their spending (though one of the partners may have retired)
            ...
            if (partner1_shortfall := max(previous_funds.partner1_funds.rrsp_savings + partner1_contribution, raw_savings / 2)) < 0:
            ...
            if (partner2_shortfall := max(previous_funds.partner2_funds.rrsp_savings + partner2_contribution, raw_savings / 2)) < 0:
            ...
            return deltas.update_partner1_deltas(deltas.partner1_deltas.update_rrsp(partner1_contribution)).update_partner2_deltas(deltas.partner2_deltas.update_rrsp(partner2_contribution))
