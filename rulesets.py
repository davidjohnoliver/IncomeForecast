...
def ampere(
    ...
    return (
        ruleset.get_career_rules(
            ...
            spending_rules.get_luxury_over_basic(
                base_spending, spending_luxury_compound_rate
            ),
            savings_rules.get_simple_linear(
                initial_rrsp_allotment,
                final_rrsp_allotment,
                initial_year,
                career_length_yrs,
            ),
            rrsp_interest_rate,
        ...
        ruleset.get_retirement_rules(
            retirement_income,
            savings_rules.get_simple_retirement_deduction(
...
def bose(
    ...
    return (
        ruleset.get_career_rules(
            salary_rules.get_compound_plateau(salary_compound_rate, salary_plateau),
            spending_rules.get_luxury_over_basic_capped(
                base_spending, spending_luxury_compound_rate, cap_fractional
            ),
            savings_rules.get_simple_linear(
                initial_rrsp_allotment,
                final_rrsp_allotment,
...
def curie(
    ...
    initial_rrsp_func = optimize.subscribe_optimized_scalar(
        "initial_rrsp", lower_bound=0, upper_bound=1, initial_guess=0.5
    )
    final_rrsp_func = optimize.subscribe_optimized_scalar(
        "final_rrsp", lower_bound=0, upper_bound=1, initial_guess=0.5
    )
    initial_rrsp_retirement_func = optimize.subscribe_optimized_scalar(
        "initial_rrsp_retirement", lower_bound=0, upper_bound=1, initial_guess=0.5
    )
    final_rrsp_retirement_func = optimize.subscribe_optimized_scalar(
        "final_rrsp_retirement", lower_bound=0, upper_bound=1, initial_guess=0.5
    )
    ...
    return (
        ruleset.get_career_rules(
            ...
            savings_rules.get_simple_linear_func(
                initial_rrsp_func,
                final_rrsp_func,
                initial_year,
                career_length_yrs,
...
def einstein(
    ...
    return (
        ruleset.get_career_rules(
            salary_rules.get_compound_plateau(salary_compound_rate, salary_plateau),
            spending_rules.get_increasing_savings_increasing_spending(
                initial_year, increase_savings_weight, False
            ),
            savings_rules.get_simple_linear(
                initial_rrsp_allotment,
                final_rrsp_allotment,
...
def galileo(
    ...
    initial_rrsp_retirement_func = optimize.subscribe_optimized_scalar(
        "initial_rrsp_retirement", lower_bound=0, upper_bound=1, initial_guess=0.05
    )
    final_rrsp_retirement_func = optimize.subscribe_optimized_scalar(
        "final_rrsp_retirement", lower_bound=0, upper_bound=1, initial_guess=0.05
    )
    ...
    return (
        ruleset.get_career_rules(
            salary_rules.get_compound_plateau(salary_compound_rate, salary_plateau),
            spending_rules.get_increasing_savings_increasing_spending(
                initial_year, increase_savings_weight, True
            ),
            savings_rules.get_simple_linear_func(
                initial_rrsp_func,
                final_rrsp_func,
...
def hawking(
    ...
    initial_rrsp_func = optimize.subscribe_optimized_scalar(
        ...
        lower_bound=0,
        upper_bound=1,
        initial_guess=initial_rrsp_allotment_guess,
    )
    final_rrsp_func = optimize.subscribe_optimized_scalar(
        "final_rrsp",
        lower_bound=0,
        upper_bound=1,
        initial_guess=final_rrsp_allotment_guess,
    )
    rrsp_retirement_func = optimize.subscribe_optimized_scalar(
        "rrsp_retirement_adjustment",
        lower_bound=-1,
        upper_bound=1,
        initial_guess=rrsp_retirement_adjustment_guess,
    )
    ...
    return (
        ...
        ruleset.get_retirement_rules(
            retirement_income,
            savings_rules.get_adjusted_heuristic_retirement_deduction(
                year_of_retirement, year_of_death, rrsp_retirement_func
            ),
            rrsp_interest_rate,
            tfsa_interest_rate,
            0,  # Unregistered interest not supported
