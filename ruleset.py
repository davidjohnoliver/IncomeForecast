...
def get_career_rules(
    ...
    return [
        ...
        natural_rules.get_calculate_investment_interest(
            rrsp_interest_rate, tfsa_interest_rate, unregistered_interest_rate
        ),
...
def get_retirement_rules(
    ...
    return [
        ...
        natural_rules.get_calculate_investment_interest(
            rrsp_interest_rate, tfsa_interest_rate, unregistered_interest_rate
        ),  # Earn interest on remaining savings
    ]


def get_couple_ruleset(
    partner1_salary_rule,
    partner2_salary_rule,
    spending_rule,
    savings_rule,
    rrsp_interest_rate: float,
    tfsa_interest_rate: float,
    ...
    def ruleset(
        ...
        for i in range(1, 3):
            ...
            yield model.get_couple_rule_from_single_rule(
                natural_rules.apply_tax_refund, i
            )
            yield model.get_couple_rule_from_single_rule(
                natural_rules.get_calculate_investment_interest(
                    rrsp_interest_rate, tfsa_interest_rate, unregistered_interest_rate
